...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (isset($_POST['email']) && isset($_POST['password']) && isset($_POST['accessoryID'])) {

      $db = new DbOperation();

      if ($db->userLogin($db->noHTML($_POST['email']), $db->noHTML($_POST['password']))) {

          $result = $db->checkinAccessory($db->noHTML($_POST['accessoryID']));

          if ($result == false) {

              $response['error'] = true;
              $response['message'] = "Accessory ID Has Not Been Checked Out";
        ...
        $response['error'] = true;
        $response['message'] = "All Fields Are Required To Book Accessory In";
