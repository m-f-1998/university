...
public class HungarianAlgorithm {
    ...
    public HungarianAlgorithm(double[][] distance_matrix) {
        if (distance_matrix.length != distance_matrix[0].length) {
            try {
        ...
        squareInRow = new int[distance_matrix.length];       // squareInRow & squareInCol indicate the position
        squareInCol = new int[distance_matrix[0].length];    // of the marked zeroes

        rowIsCovered = new int[distance_matrix.length];      // indicates whether a row is covered
        colIsCovered = new int[distance_matrix[0].length];   // indicates whether a column is covered
        staredZeroesInRow = new int[distance_matrix.length]; // storage for the 0*
