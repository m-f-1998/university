          ...
          success: function( data ) {
            if ( data != undefined ) {
              ...
              if ( data[ "error" ] )
                ...
                if ( data[ "account" ][ 0 ][ "is_admin" ] == 1 ) {
                  if ( data[ "account" ][ 0 ][ "email_verified" ] == 1 ) {
                    delete data[ "account" ][ 0 ][ "email_verified" ]; // Delete Flags So Not To Be Stored Locally
                    delete data[ "account" ][ 0 ][ "is_admin" ];
                    delete data[ "account" ][ 0 ][ "is_lecturer" ];
                    $.redirect( './', { account: data[ "account" ][ 0 ], code: data[ "code" ] } );
                  } else {
                    document.getElementById( 'error' ).innerHTML =  "Please Verify Your Email Before You Login";
                  }
                } else {
                  document.getElementById( 'error' ).innerHTML =  "You Must Be An Administrator To Use This Portal";
                }
              }
            }
          }
